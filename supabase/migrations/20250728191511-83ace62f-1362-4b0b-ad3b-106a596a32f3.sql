-- تصحيح دالة حساب الأرباح لتتعامل مع حالات الدفع المختلفة
...
BEGIN
  ...
  -- حساب المستحقات المدفوعة من جدول المصاريف
  paid_employee_dues AS (
  ...
  SELECT 
    ...
    SUM(fd.capital_amount) + SUM(fd.revenue_amount) - SUM(fd.expense_amount) - SUM(fd.purchase_amount) - ped.paid_amount
  FROM financial_data fd, paid_employee_dues ped;
END;
$function$;