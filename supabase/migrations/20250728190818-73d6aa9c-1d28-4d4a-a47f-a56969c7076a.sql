-- تصحيح منطق حساب الأرباح نهائياً
-- ربح النظام = ربح طلبات المدير كاملاً + ربح النظام من طلبات الموظفين (بعد خصم مستحقاتهم)
CREATE OR REPLACE FUNCTION public.calculate_enhanced_main_cash_balance_v6()
RETURNS TABLE(
...
BEGIN
  ...
  WITH financial_data AS (
    ...
    -- إجمالي مستحقات الموظفين (من جدول الأرباح) - توزيع للربح وليس مصروف
    SELECT 
      0::numeric,
      0::numeric,
      0::numeric,
    ...
    -- ربح النظام = إجمالي الربح - مستحقات الموظفين
    SELECT 
      0::numeric,
      0::numeric,
      0::numeric,
  ...
  SELECT 
    ...
    SUM(fd.employee_profit_amount), -- مستحقات الموظفين (توزيع للربح)
    ...
    SUM(fd.capital_amount) + SUM(fd.revenue_amount) - SUM(fd.expense_amount) - SUM(fd.purchase_amount) - (
      SELECT COALESCE(SUM(amount), 0) FROM expenses 
      WHERE status = 'approved' AND expense_type = 'system' AND category = 'مستحقات الموظفين'
    )
  FROM financial_data fd;
END;
$function$;

-- تحديث الدالة الأساسية لتستخدم النسخة المصححة
...
AS $function$
BEGIN
  RETURN QUERY SELECT * FROM calculate_enhanced_main_cash_balance_v6();
END;
$function$;