...
-- إنشاء دالة لتوليد QR ID فريد للطلبات (مصححة)
...
BEGIN
  LOOP
    ...
    final_qr_id := qr_prefix || random_suffix;
    
    -- التحقق من عدم وجود هذا الـ QR ID
    IF NOT EXISTS (SELECT 1 FROM orders WHERE orders.qr_id = final_qr_id) THEN
      RETURN final_qr_id;
    END IF;
    ...
    IF counter > 1000 THEN
      ...
      final_qr_id := qr_prefix || EXTRACT(EPOCH FROM now())::bigint || '-' || random_suffix;
      RETURN final_qr_id;
    END IF;
  END LOOP;
END;
