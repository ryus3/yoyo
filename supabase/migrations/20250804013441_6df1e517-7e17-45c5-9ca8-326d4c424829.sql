-- إصلاح دالة الإحصائيات الموحدة لاستخدام المدن بدلاً من المحافظات الفارغة
...
BEGIN
  ...
  SELECT 
    ...
    -- أفضل المدن (أول 10) - استخدام customer_city بدلاً من customer_province
    (
      SELECT jsonb_agg(
        jsonb_build_object(
          'province', province_stats.customer_city,
