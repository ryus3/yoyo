...
BEGIN
    ...
    SELECT current_balance, name INTO current_balance, source_name
    FROM public.cash_sources
    WHERE id = p_cash_source_id AND is_active = true;
    ...
    ELSIF p_movement_type = 'out' THEN
        ...
        IF new_balance < 0 AND NOT (source_name = 'القاصة الرئيسية' AND p_reference_type = 'capital_withdrawal') THEN
            RAISE EXCEPTION 'رصيد غير كافي للسحب. الرصيد الحالي: %', current_balance;
        END IF;
    ELSE
