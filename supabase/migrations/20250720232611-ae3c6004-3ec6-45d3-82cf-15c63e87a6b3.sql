-- تحديث دالة حساب الأرباح عند استلام الفاتورة لتستخدم قواعد الربح المحددة
...
BEGIN
  ...
  IF NEW.receipt_received = true AND (OLD.receipt_received = false OR OLD.receipt_received IS NULL) THEN
    ...
    ELSE
      ...
      IF employee_rules IS NOT NULL THEN
    ...
    INSERT INTO public.profits (
      ...
      manager_profit,
      CASE 
        WHEN employee_rules IS NOT NULL AND employee_rules.rule_type = 'percentage' 
        THEN employee_rules.profit_percentage 
        ELSE 0 
      END,
      employee_profit,
      'invoice_received'
    ...
    -- الحصول على معرف القاصة الرئيسية وتحديث رصيدها
    ...
    IF main_cash_id IS NOT NULL THEN
      ...
      SET current_balance = public.calculate_main_cash_balance(),
          updated_at = now()
      WHERE id = main_cash_id;
    END IF;
    
    RAISE NOTICE 'تم حساب الأرباح للطلب %: إيرادات=%, تكلفة=%, ربح مدير=%, ربح موظف=%', 
