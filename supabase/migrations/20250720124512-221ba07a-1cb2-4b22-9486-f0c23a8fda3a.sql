-- حذف الترايغر أولاً ثم الدالة
DROP TRIGGER IF EXISTS trigger_update_main_cash_on_capital_change ON public.settings;
DROP FUNCTION IF EXISTS public.update_main_cash_on_capital_change() CASCADE;
...
BEGIN
  ...
  IF NEW.key = 'initial_capital' AND OLD.key = 'initial_capital' THEN
    ...
    -- تحديث الرصيد الابتدائي والحالي للقاصة الرئيسية
    ...
    RAISE NOTICE 'تم تحديث رأس المال والرصيد بنجاح: رأس المال=%, حركات أخرى=%, المجموع=%', 
                 new_capital_value, other_movements_total, (new_capital_value + other_movements_total);
