-- إصلاح دالة الإحصائيات الموحدة لتصحيح أسماء الأعمدة
...
BEGIN
  ...
  SELECT 
    ...
    -- أفضل المحافظات (أول 10) - تصحيح اسم العمود
    (
      SELECT jsonb_agg(
        jsonb_build_object(
          'province', province_stats.customer_province,
      ...
      FROM (
        ...
        FROM orders 
        WHERE status = 'completed' AND receipt_received = true
        AND customer_province IS NOT NULL AND customer_province != ''
