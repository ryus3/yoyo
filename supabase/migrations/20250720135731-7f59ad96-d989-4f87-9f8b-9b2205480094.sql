-- تحديث دالة إضافة الأرباح لتستخدم رأس المال الصحيح من الإعدادات
...
BEGIN
  ...
  IF OLD.receipt_received IS DISTINCT FROM NEW.receipt_received 
    ...
    SELECT COALESCE((value::numeric), 10000000) INTO current_capital
...
-- حذف أي حركات نقدية مكررة للمصاريف إذا كانت موجودة
DELETE FROM cash_movements 
WHERE reference_type = 'expense' 
AND id NOT IN (
    SELECT DISTINCT ON (reference_id, amount, description) id
    FROM cash_movements 
    WHERE reference_type = 'expense'
    ORDER BY reference_id, amount, description, created_at
);