-- تنظيف الحركات المكررة وتصحيح الرصيد
DO $$ 
DECLARE
  main_cash_id UUID;
...
BEGIN
  ...
  SELECT cs.current_balance INTO current_balance_val 
  FROM cash_sources cs
  WHERE cs.id = main_cash_id;
  
  -- تصحيح الرصيد إذا كان مختلفاً
  IF current_balance_val != correct_balance THEN
    UPDATE cash_sources 
    ...
    INSERT INTO cash_movements (
      ...
      main_cash_id,
      ABS(correct_balance - current_balance_val),
      CASE WHEN correct_balance > current_balance_val THEN 'in' ELSE 'out' END,
      'adjustment',
    ...
    RAISE NOTICE 'تم تصحيح الرصيد من % إلى %', current_balance_val, correct_balance;
  END IF;
