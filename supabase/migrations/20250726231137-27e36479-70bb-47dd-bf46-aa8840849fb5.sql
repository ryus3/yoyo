-- إكمال النظام مع الدوال والبيانات

-- 1. تحديث صلاحية النقاط في جدول customer_loyalty
ALTER TABLE public.customer_loyalty 
ADD COLUMN IF NOT EXISTS points_expiry_date TIMESTAMP WITH TIME ZONE;

-- 2. دالة لإنشاء البروموكود التلقائي
...
-- 3. دالة للتحقق من البروموكود وتطبيق الخصم
...
-- 4. دالة لتحليل جنس العميل بناءً على مشترياته
...
-- 5. إدراج تصنيفات المنتجات الافتراضية (رجالي/نسائي)
...
-- 6. تحديث صلاحية النقاط للعملاء الحاليين
...
-- 7. إنشاء البروموكود للعملاء الحاليين
...
-- 8. تحليل جنس العملاء الحاليين (افتراضي)
INSERT INTO customer_gender_segments (customer_id, gender_type, confidence_score)
